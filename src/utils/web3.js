...
export default () => {
  ...
  return web3.eth.getBlockNumber().then(number => {
    store.dispatch({ type: INITIAL_BLOCK, payload: number }); // Saving last block in store (needed for pagination)
    store.dispatch(push(`/${number}/0`));
  });
};
